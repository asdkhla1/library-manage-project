...
利用sql文件，创建数据库和表，并使用IDEA进行数据库连接

library-front   library-behind  分别对应前端和后端代码

前端代码导入后，可以使用npm i 安装相关依赖，后端的话同理，根据pom.xml文件来构建maven依赖
...
E-R 图包含以下实体：读者种类、读者、书籍种类、书籍、借还书记录、管理员。各实体之间通过外键建立关联关系。
...
采用ECharts可视化库，来进行各种数据图表的绘制，包括图书读者分类数量，借书还书统计以及人气书籍Top排行榜等。
...
进入借书页面，页面下方会分页展示读者列表和图书列表，其中图书都是当前处于图书馆中未被出借的书。管理员可以单选其中一个读者和一本书籍，表示借书，对应的上方表单会回显对应的读者和图书的id，再选择借书日期后便可进行借书记录的新增操作。进入记录展示页面，会分页显示所有借还书记录，同时状态栏还会以不同颜色标准不同的还书状态。进行还书时，需要选择还书日期（在借书日期之后且不能晚于当前日期），填写完相关信息后即可还书，本页面下面还有借书人和对应书籍的详细信息展示。
